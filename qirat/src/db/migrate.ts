 ...
 * connects as qirat_app, which owns nothing and inherits nothing.
 */
...
export interface MigrateOptions {
  adminUrl: string;
  /** Password given to qirat_app; the app's own DATABASE_URL must match. */
  appPassword: string;
...
export async function migrate({ adminUrl, appPassword, log = () => {} }: MigrateOptions) {
  ...
  try {
    ...
    await sql`create schema if not exists qirat`;
    await sql`
      create table if not exists qirat.schema_migrations (
    ...
    const applied = new Set(
      (await sql<{ version: string }[]>`select version from qirat.schema_migrations`).map(
        (row) => row.version,
      ),
    );
    ...
    for (const file of files) {
      ...
      await sql.begin(async (tx) => {
        ...
        await tx`insert into qirat.schema_migrations (version) values (${file})`;
      });
      log(`  + ${file}`);
    }
...
async function grantSchemaUsage(sql: postgres.Sql) {
  ...
  for (const role of roles) {
    await sql.unsafe(await statement(sql, 'grant usage on schema qirat to %I', role));
  }
}
...
async function main() {
  ...
  const appPassword = process.env.QIRAT_APP_DB_PASSWORD;
  if (!adminUrl || !appPassword) {
    console.error('Set ADMIN_DATABASE_URL and QIRAT_APP_DB_PASSWORD. See qirat/.env.example.');
    process.exit(1);
  }
