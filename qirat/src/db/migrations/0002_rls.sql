...
--
-- The design in one paragraph: the application connects as `qirat_app`, which is
...
grant usage on schema public to qirat_role_owner, qirat_role_manager, qirat_role_member, qirat_role_partner, qirat_bootstrap;
grant usage on schema qirat  to qirat_role_owner, qirat_role_manager, qirat_role_member, qirat_role_partner, qirat_app, qirat_bootstrap;
...
grant select, update on organizations to qirat_role_owner;
...
grant select (id, org_id, slug, name, name_ar, default_locale, numbering_system, created_at)
  on organizations to qirat_role_member, qirat_role_partner;
...
grant insert on organizations to qirat_role_owner;

create policy organizations_owner_rw on organizations
  for all to qirat_role_owner
  using (id = qirat.current_org_id())
  with check (id = qirat.current_org_id());

create policy organizations_read on organizations
  for select to qirat_role_manager, qirat_role_member, qirat_role_partner
  using (id = qirat.current_org_id());

-- --- users -------------------------------------------------------------------

-- password_hash is granted to nobody at all. The only thing that reads it is
-- qirat.authenticate_lookup, which runs as qirat_bootstrap.
grant select (id, org_id, email, name, role, locale, is_active, created_at, last_login_at)
  on users to qirat_role_owner, qirat_role_manager;
grant select (id, org_id, email, name, role, locale, is_active, created_at, last_login_at)
  on users to qirat_role_member, qirat_role_partner;
grant insert, update, delete on users to qirat_role_owner;
grant update (last_login_at) on users to qirat_role_manager, qirat_role_member, qirat_role_partner;
...
create policy users_self_read on users
  for select to qirat_role_member, qirat_role_partner
  using (org_id = qirat.current_org_id() and id = qirat.current_user_id());

create policy users_self_touch on users
  for update to qirat_role_manager, qirat_role_member, qirat_role_partner
  using (org_id = qirat.current_org_id() and id = qirat.current_user_id())
  with check (org_id = qirat.current_org_id() and id = qirat.current_user_id());

-- --- brand kits --------------------------------------------------------------

grant select, insert, update on brand_kits to qirat_role_owner;
grant select on brand_kits to qirat_role_manager, qirat_role_member, qirat_role_partner;
...
grant select, insert, update, delete on services to qirat_role_owner;
-- An account manager needs the whole band: it is what the pricing slider moves
-- within, and it is internal by design.
grant select on services to qirat_role_manager;
...
grant select (id, org_id, name, name_ar, task_template, is_active, created_at)
  on services to qirat_role_member;
...
create policy services_read on services
  for select to qirat_role_manager, qirat_role_member
  using (org_id = qirat.current_org_id());

-- --- clients -----------------------------------------------------------------

grant select, insert, update, delete on clients to qirat_role_owner;
grant select, insert, update on clients to qirat_role_manager;
grant select (id, org_id, name, name_ar, country, created_at) on clients to qirat_role_member;
...
grant select, insert, update, delete on deals to qirat_role_owner;
grant select, insert, update on deals to qirat_role_manager;
...
create policy deals_manager_rw on deals
  for all to qirat_role_manager
  using (org_id = qirat.current_org_id() and owner_user_id = qirat.current_user_id())
  with check (org_id = qirat.current_org_id() and owner_user_id = qirat.current_user_id());

-- --- audit log ---------------------------------------------------------------

-- Everyone appends; only the Owner reads. Nobody updates or deletes — see 0003.
grant insert on audit_log
  to qirat_role_owner, qirat_role_manager, qirat_role_member, qirat_role_partner;
grant select on audit_log to qirat_role_owner;

create policy audit_append on audit_log
  for insert to qirat_role_owner, qirat_role_manager, qirat_role_member, qirat_role_partner
  with check (org_id = qirat.current_org_id());

create policy audit_owner_read on audit_log
  for select to qirat_role_owner
  using (org_id = qirat.current_org_id());
...
--
-- It runs as qirat_bootstrap, which cannot log in, holds no other privilege, and
-- is reachable only through this function. Everything else in the system —
-- signup included — runs under ordinary tenant context.
create or replace function qirat.authenticate_lookup(p_email text)
returns table (
  user_id       uuid,
  org_id        uuid,
  org_slug      text,
...
grant select on users, organizations to qirat_bootstrap;

create policy users_bootstrap_read on users
  for select to qirat_bootstrap using (true);
