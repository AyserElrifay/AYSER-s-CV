...
create or replace function qirat.reject_mutation() returns trigger
language plpgsql as $$
begin
...
create trigger audit_log_no_update before update on audit_log
  for each row execute function qirat.reject_mutation();
...
create trigger audit_log_no_truncate before truncate on audit_log
  for each statement execute function qirat.reject_mutation();
...
create or replace function qirat.protect_frozen_deal_terms() returns trigger
language plpgsql as $$
begin
...
create trigger deals_freeze_on_close before update on deals
  for each row execute function qirat.protect_frozen_deal_terms();

-- --- updated_at --------------------------------------------------------------

create or replace function qirat.touch_updated_at() returns trigger
language plpgsql as $$
begin
  new.updated_at := now();
  return new;
