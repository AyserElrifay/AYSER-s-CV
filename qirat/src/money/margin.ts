 ...
 *
 * `below-floor` is deliberately not "the margin dropped under some percentage".
 * It means the price went under the floor the owner set for this service. That
 * is a line the agency drew itself, so crossing it is a fact rather than a
 * judgement, and it is the one state that changes what the interface lets you
 * do next.
 */
export type MarginSignal = 'healthy' | 'thin' | 'below-floor' | 'unpriced';

export interface MarginThresholds {
  /** At or above this margin, the reading is healthy. */
  readonly healthyFromBp: BasisPoints;
  /**
   * Only consulted when a deal has no price band to compare against — a
   * one-off with no service attached. With a band, the floor decides.
   */
...
/**
 * How the margin itself is doing — a different question from where the price
 * sits relative to the floor, and it must be answered separately.
 *
 * A deal can be sold under the floor and still carry a 64% margin. Colouring
 * that margin red because the price broke a policy makes the bar say something
 * untrue: it is measuring profit, and the profit is fine. The floor is a fact
 * about the price, and it is reported on the price.
 *
 * So the card carries both: the price reads red and the button changes, while
 * the bar goes on reporting the margin honestly. Two facts, neither of them
 * distorted to fit the other.
 */
export type MarginHealth = 'healthy' | 'thin' | 'loss' | 'unpriced';
...
export function marginSignal(input: {
  readonly marginBasisPoints: number | null;
  /** The agreed price. Given with `band`, the floor decides the red state. */
  readonly price?: Money;
  readonly band?: PriceBand;
  readonly thresholds?: MarginThresholds;
}): MarginSignal {
  const { marginBasisPoints: bp, price, band } = input;
  const thresholds = input.thresholds ?? DEFAULT_MARGIN_THRESHOLDS;

  // The floor outranks everything. A deal can carry a healthy-looking margin
  // and still be sold under the price the owner said never to go under —
  // usually because the cost estimate is optimistic, which is exactly when the
  // interface should be saying so.
  if (price && band && requiresOwnerApproval(price, band)) return 'below-floor';

  if (bp === null) return 'unpriced';
  if (bp >= thresholds.healthyFromBp) return 'healthy';
  // No band to consult: fall back to the margin threshold for the red state.
  if (!band && bp < thresholds.warningFromBp) return 'below-floor';
  return 'thin';
}
 ...
 *
 * Below the floor a deal does not close, and it also does not fail: it goes to
 * the owner. The interface changes its wording rather than throwing an error,
 * so the limit reads as a door rather than a wall — but the decision itself
 * belongs here, on the server's side of the line, because the browser knows the
 * band and could therefore lie about it.
 *
 * A deal with no band has no floor to be under and simply closes.
 */
export type CloseRoute = 'close' | 'owner-approval';

export function routeForClose(price: Money, band: PriceBand | null): CloseRoute {
  if (!band) return 'close';
  if (price.currency !== band.floor.currency) {
    // A band quoted in another currency cannot be compared to this price, so
    // there is nothing to be under.
    return 'close';
  }
  return requiresOwnerApproval(price, band) ? 'owner-approval' : 'close';
}
