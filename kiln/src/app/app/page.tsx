  ...
  return (
    <div className="space-y-10">
      <section>
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          {cards.length === 0 ? (
            ...
            cards.map((deal) => <DealCard key={deal.id} deal={deal} locale={locale} />)
          )}
        </div>
      </section>
      ...
      <section>
        ...
        <ul className="mt-4 max-w-2xl divide-y divide-line rounded-[10px] border border-line bg-surface">
          {bands.map((band) => (
            <li key={band.id} className="flex items-baseline justify-between gap-4 px-4 py-3">
              ...
              <span className="tabular text-[13px] text-ink-soft">
                ...
                <span className="font-medium text-ink">
                  {formatMoney(money(band.targetMinor, band.currency), { locale, display: 'none' })}
                </span>
                {' · '}
                {formatMoney(money(band.ceilingMinor, band.currency), { locale, display: 'none' })}
              </span>
            </li>
          ))}
        </ul>
      ...
      {user.role === 'owner' && (
        <section>
          ...
          {audit.length === 0 ? (
            ...
            <ul className="mt-4 max-w-2xl divide-y divide-line rounded-[10px] border border-line bg-surface text-[13px]">
              {audit.map((entry) => (
