 ...
 * connects as kiln_app, which owns nothing and inherits nothing.
 */
...
export interface MigrateOptions {
  adminUrl: string;
  /** Password given to kiln_app; the app's own DATABASE_URL must match. */
  appPassword: string;
...
export async function migrate({ adminUrl, appPassword, log = () => {} }: MigrateOptions) {
  ...
  try {
    ...
    await sql`create schema if not exists kiln`;
    await sql`
      create table if not exists kiln.schema_migrations (
    ...
    const applied = new Set(
      (await sql<{ version: string }[]>`select version from kiln.schema_migrations`).map(
        (row) => row.version,
      ),
    );
    ...
    for (const file of files) {
      ...
      await sql.begin(async (tx) => {
        ...
        await tx`insert into kiln.schema_migrations (version) values (${file})`;
      });
      log(`  + ${file}`);
    }
...
async function grantSchemaUsage(sql: postgres.Sql) {
  ...
  for (const role of roles) {
    await sql.unsafe(await statement(sql, 'grant usage on schema kiln to %I', role));
  }
}
...
async function main() {
  ...
  const appPassword = process.env.KILN_APP_DB_PASSWORD;
  if (!adminUrl || !appPassword) {
    console.error('Set ADMIN_DATABASE_URL and KILN_APP_DB_PASSWORD. See kiln/.env.example.');
    process.exit(1);
  }
