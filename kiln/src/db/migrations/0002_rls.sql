...
--
-- The design in one paragraph: the application connects as `kiln_app`, which is
...
grant usage on schema public to kiln_role_owner, kiln_role_manager, kiln_role_member, kiln_role_partner, kiln_bootstrap;
grant usage on schema kiln  to kiln_role_owner, kiln_role_manager, kiln_role_member, kiln_role_partner, kiln_app, kiln_bootstrap;
...
grant select, update on organizations to kiln_role_owner;
grant select (id, org_id, slug, name, name_ar, default_currency, house_rate_bp,
  ...
  on organizations to kiln_role_manager;
...
grant select (id, org_id, slug, name, name_ar, default_locale, numbering_system, created_at)
  on organizations to kiln_role_member, kiln_role_partner;
...
grant insert on organizations to kiln_role_owner;

create policy organizations_owner_rw on organizations
  for all to kiln_role_owner
  using (id = kiln.current_org_id())
  with check (id = kiln.current_org_id());

create policy organizations_read on organizations
  for select to kiln_role_manager, kiln_role_member, kiln_role_partner
  using (id = kiln.current_org_id());

-- --- users -------------------------------------------------------------------

-- password_hash is granted to nobody at all. The only thing that reads it is
-- kiln.authenticate_lookup, which runs as kiln_bootstrap.
grant select (id, org_id, email, name, role, locale, is_active, created_at, last_login_at)
  on users to kiln_role_owner, kiln_role_manager;
grant select (id, org_id, email, name, role, locale, is_active, created_at, last_login_at)
  on users to kiln_role_member, kiln_role_partner;
grant insert, update, delete on users to kiln_role_owner;
grant update (last_login_at) on users to kiln_role_manager, kiln_role_member, kiln_role_partner;

create policy users_owner_rw on users
  for all to kiln_role_owner
  using (org_id = kiln.current_org_id())
  with check (org_id = kiln.current_org_id());

create policy users_manager_read on users
  for select to kiln_role_manager
  using (org_id = kiln.current_org_id());

-- A Member or Partner sees exactly one user: themselves.
create policy users_self_read on users
  for select to kiln_role_member, kiln_role_partner
  using (org_id = kiln.current_org_id() and id = kiln.current_user_id());

create policy users_self_touch on users
  for update to kiln_role_manager, kiln_role_member, kiln_role_partner
  using (org_id = kiln.current_org_id() and id = kiln.current_user_id())
  with check (org_id = kiln.current_org_id() and id = kiln.current_user_id());

-- --- brand kits --------------------------------------------------------------

grant select, insert, update on brand_kits to kiln_role_owner;
grant select on brand_kits to kiln_role_manager, kiln_role_member, kiln_role_partner;
...
grant select (id, org_id, name, name_ar, task_template, is_active, created_at)
  on services to kiln_role_member;
...
create policy services_read on services
  for select to kiln_role_manager, kiln_role_member
  using (org_id = kiln.current_org_id());

-- --- clients -----------------------------------------------------------------

grant select, insert, update, delete on clients to kiln_role_owner;
grant select, insert, update on clients to kiln_role_manager;
grant select (id, org_id, name, name_ar, country, created_at) on clients to kiln_role_member;
...
create policy clients_manager_rw on clients
  for all to kiln_role_manager
  using (org_id = kiln.current_org_id())
  with check (org_id = kiln.current_org_id());

create policy clients_member_read on clients
  for select to kiln_role_member
  using (org_id = kiln.current_org_id());

-- --- deals -------------------------------------------------------------------

grant select, insert, update, delete on deals to kiln_role_owner;
grant select, insert, update on deals to kiln_role_manager;
...
create policy deals_manager_rw on deals
  for all to kiln_role_manager
  using (org_id = kiln.current_org_id() and owner_user_id = kiln.current_user_id())
  with check (org_id = kiln.current_org_id() and owner_user_id = kiln.current_user_id());

-- --- audit log ---------------------------------------------------------------

-- Everyone appends; only the Owner reads. Nobody updates or deletes — see 0003.
grant insert on audit_log
  to kiln_role_owner, kiln_role_manager, kiln_role_member, kiln_role_partner;
grant select on audit_log to kiln_role_owner;

create policy audit_append on audit_log
  for insert to kiln_role_owner, kiln_role_manager, kiln_role_member, kiln_role_partner
  with check (org_id = kiln.current_org_id());

create policy audit_owner_read on audit_log
  for select to kiln_role_owner
  using (org_id = kiln.current_org_id());
...
--
-- It runs as kiln_bootstrap, which cannot log in, holds no other privilege, and
-- is reachable only through this function. Everything else in the system —
-- signup included — runs under ordinary tenant context.
create or replace function kiln.authenticate_lookup(p_email text)
returns table (
  user_id       uuid,
  org_id        uuid,
  org_slug      text,
...
grant select on users, organizations to kiln_bootstrap;

create policy users_bootstrap_read on users
  for select to kiln_bootstrap using (true);
...
alter function kiln.authenticate_lookup(text) owner to kiln_bootstrap;

revoke all on function kiln.authenticate_lookup(text) from public;
grant execute on function kiln.authenticate_lookup(text) to kiln_app;
