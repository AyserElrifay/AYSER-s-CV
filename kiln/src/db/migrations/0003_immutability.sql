...
create trigger audit_log_no_truncate before truncate on audit_log
  for each statement execute function kiln.reject_mutation();
...
create or replace function kiln.protect_frozen_deal_terms() returns trigger
language plpgsql as $$
begin
...
create trigger deals_freeze_on_close before update on deals
  for each row execute function kiln.protect_frozen_deal_terms();

-- --- updated_at --------------------------------------------------------------

create or replace function kiln.touch_updated_at() returns trigger
language plpgsql as $$
begin
  new.updated_at := now();
  return new;
...
create trigger deals_touch before update on deals
  for each row execute function kiln.touch_updated_at();
