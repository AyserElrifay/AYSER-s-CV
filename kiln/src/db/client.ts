 ...
 *  - A query that forgets to switch roles is refused outright, because kiln_app
...
function connectionUrl(): string {
  ...
  if (!url) throw new Error('DATABASE_URL is not set. Copy kiln/.env.example to kiln/.env.');
  return url;
}
...
export async function findLoginCandidates(email: string): Promise<LoginCandidate[]> {
  const rows = await getDb().execute<{
    ...
    user_name: string;
    locale: string;
  }>(raw`select * from kiln.authenticate_lookup(${email})`);
  ...
  if (faults.length > 0) {
    throw new TenantContextError(
      `DATABASE_URL connects as "${row.role_name}", which ${faults.join(' and ')}. ` +
        'Point it at kiln_app instead — see kiln/.env.example.',
    );
  }
}
