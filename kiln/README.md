...
**Phase 0 is complete.** Auth, organisations, four enforced roles, and tenant
isolation proved by an automated suite. The money module is built and tested
ahead of Phase 1 because everything downstream depends on it being right.

`../` in this repository is a different project (`moments-superapp`). Kiln lives
...
```bash
sudo -u postgres psql -c "create database kiln_dev;"
sudo -u postgres psql -c "create database kiln_test;"
...
migration creates `kiln_app` and the four role-specific roles itself.
...
213 tests, about eleven seconds.

| Suite | What it holds down |
|---|---|
| `src/money/*.test.ts` | 121 tests. Every function, negative margins, zero-revenue deals, currency mismatches, three-decimal and zero-decimal currencies, amounts past 2^53, and a thousand generated splits that must each sum exactly. |
...
3. Run the migration once from your machine:
   ...
   ```bash
   ADMIN_DATABASE_URL='postgres://…' KILN_APP_DB_PASSWORD='<a long random string>' \
     npm run db:migrate
   ```

   This creates `kiln_app` and the four role-specific roles. `DATABASE_URL` is
   then the same host and database as the admin URL, but as `kiln_app` with the
   password you just set.

### 2. Application — Vercel

1. Import this repository at [vercel.com](https://vercel.com).
2. **Set Root Directory to `kiln`.** The repository root is a different project.
3. Environment variables:

   | Name | Value |
   |---|---|
   | `DATABASE_URL` | the `kiln_app` connection string, **pooled** |
   | `AUTH_SECRET` | `openssl rand -base64 32` |
   | `AUTH_TRUST_HOST` | `true` |

   `ADMIN_DATABASE_URL` and `KILN_APP_DB_PASSWORD` are **not** set on Vercel.
...
src/
  money/          The money module. Pure, no I/O, exhaustively tested.
                  Every piece of money arithmetic in Kiln is in here.
  db/
...
- **The margin bar reports; it does not yet respond.** It is rendered from the
  money module and is correct. Dragging the price, with the weight and
  resistance the brief asks for, is Phase 1.
- **No deal editing.** Deals are created by onboarding. Creating and pricing
  them is Phase 1.
