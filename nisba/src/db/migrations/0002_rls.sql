...
--
-- The design in one paragraph: the application connects as `nisba_app`, which is
...
grant usage on schema public to nisba_role_owner, nisba_role_manager, nisba_role_member, nisba_role_partner, nisba_bootstrap;
grant usage on schema nisba  to nisba_role_owner, nisba_role_manager, nisba_role_member, nisba_role_partner, nisba_app, nisba_bootstrap;
...
grant select, update on organizations to nisba_role_owner;
grant select (id, org_id, slug, name, name_ar, default_currency, house_rate_bp,
  ...
  on organizations to nisba_role_manager;
...
grant select (id, org_id, slug, name, name_ar, default_locale, numbering_system, created_at)
  on organizations to nisba_role_member, nisba_role_partner;
...
grant insert on organizations to nisba_role_owner;

create policy organizations_owner_rw on organizations
  for all to nisba_role_owner
  using (id = nisba.current_org_id())
  with check (id = nisba.current_org_id());

create policy organizations_read on organizations
  for select to nisba_role_manager, nisba_role_member, nisba_role_partner
  using (id = nisba.current_org_id());

-- --- users -------------------------------------------------------------------

-- password_hash is granted to nobody at all. The only thing that reads it is
-- nisba.authenticate_lookup, which runs as nisba_bootstrap.
grant select (id, org_id, email, name, role, locale, is_active, created_at, last_login_at)
  on users to nisba_role_owner, nisba_role_manager;
grant select (id, org_id, email, name, role, locale, is_active, created_at, last_login_at)
  on users to nisba_role_member, nisba_role_partner;
grant insert, update, delete on users to nisba_role_owner;
grant update (last_login_at) on users to nisba_role_manager, nisba_role_member, nisba_role_partner;

create policy users_owner_rw on users
  for all to nisba_role_owner
  using (org_id = nisba.current_org_id())
  with check (org_id = nisba.current_org_id());

create policy users_manager_read on users
  for select to nisba_role_manager
  using (org_id = nisba.current_org_id());

-- A Member or Partner sees exactly one user: themselves.
create policy users_self_read on users
  for select to nisba_role_member, nisba_role_partner
  using (org_id = nisba.current_org_id() and id = nisba.current_user_id());

create policy users_self_touch on users
  for update to nisba_role_manager, nisba_role_member, nisba_role_partner
  using (org_id = nisba.current_org_id() and id = nisba.current_user_id())
  with check (org_id = nisba.current_org_id() and id = nisba.current_user_id());

-- --- brand kits --------------------------------------------------------------

grant select, insert, update on brand_kits to nisba_role_owner;
grant select on brand_kits to nisba_role_manager, nisba_role_member, nisba_role_partner;
...
grant select, insert, update, delete on services to nisba_role_owner;
-- An account manager needs the whole band: it is what the pricing slider moves
-- within, and it is internal by design.
grant select on services to nisba_role_manager;
...
grant select (id, org_id, name, name_ar, task_template, is_active, created_at)
  on services to nisba_role_member;
...
create policy services_read on services
  for select to nisba_role_manager, nisba_role_member
  using (org_id = nisba.current_org_id());

-- --- clients -----------------------------------------------------------------

grant select, insert, update, delete on clients to nisba_role_owner;
grant select, insert, update on clients to nisba_role_manager;
grant select (id, org_id, name, name_ar, country, created_at) on clients to nisba_role_member;
...
create policy clients_manager_rw on clients
  for all to nisba_role_manager
  using (org_id = nisba.current_org_id())
  with check (org_id = nisba.current_org_id());

create policy clients_member_read on clients
  for select to nisba_role_member
  using (org_id = nisba.current_org_id());

-- --- deals -------------------------------------------------------------------

grant select, insert, update, delete on deals to nisba_role_owner;
grant select, insert, update on deals to nisba_role_manager;
...
create policy deals_manager_rw on deals
  for all to nisba_role_manager
  using (org_id = nisba.current_org_id() and owner_user_id = nisba.current_user_id())
  with check (org_id = nisba.current_org_id() and owner_user_id = nisba.current_user_id());

-- --- audit log ---------------------------------------------------------------

-- Everyone appends; only the Owner reads. Nobody updates or deletes — see 0003.
grant insert on audit_log
  to nisba_role_owner, nisba_role_manager, nisba_role_member, nisba_role_partner;
grant select on audit_log to nisba_role_owner;

create policy audit_append on audit_log
  for insert to nisba_role_owner, nisba_role_manager, nisba_role_member, nisba_role_partner
  with check (org_id = nisba.current_org_id());

create policy audit_owner_read on audit_log
  for select to nisba_role_owner
  using (org_id = nisba.current_org_id());
...
--
-- It runs as nisba_bootstrap, which cannot log in, holds no other privilege, and
-- is reachable only through this function. Everything else in the system —
-- signup included — runs under ordinary tenant context.
create or replace function nisba.authenticate_lookup(p_email text)
returns table (
  user_id       uuid,
  org_id        uuid,
  org_slug      text,
...
grant select on users, organizations to nisba_bootstrap;

create policy users_bootstrap_read on users
  for select to nisba_bootstrap using (true);
...
alter function nisba.authenticate_lookup(text) owner to nisba_bootstrap;

revoke all on function nisba.authenticate_lookup(text) from public;
grant execute on function nisba.authenticate_lookup(text) to nisba_app;
