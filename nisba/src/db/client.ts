 ...
 *  - A query that forgets to switch roles is refused outright, because nisba_app
...
function connectionUrl(): string {
  ...
  if (!url) throw new Error('DATABASE_URL is not set. Copy nisba/.env.example to nisba/.env.');
  return url;
}
...
export async function findLoginCandidates(email: string): Promise<LoginCandidate[]> {
  const rows = await getDb().execute<{
    ...
    user_name: string;
    locale: string;
  }>(raw`select * from nisba.authenticate_lookup(${email})`);
  ...
  if (faults.length > 0) {
    throw new TenantContextError(
      `DATABASE_URL connects as "${row.role_name}", which ${faults.join(' and ')}. ` +
        'Point it at nisba_app instead — see nisba/.env.example.',
    );
  }
}
