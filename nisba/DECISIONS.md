...
`{org}.nisba.app` needs a registered domain, wildcard DNS, and a wildcard TLS
...
link needs to look like the agency rather than like Nisba.
...
**Decided:** the application connects as `nisba_app`, which is `NOINHERIT`, owns
...
| the role switch | refused: `nisba_app` has no privilege, and no schema `USAGE` |
...
**Decided:** `nisba.authenticate_lookup(email)` is `SECURITY DEFINER`, owned by
`nisba_bootstrap` — a role that cannot log in and is reachable only through that
function.
...
its owner cannot log in, and that only `nisba_app` may execute it.
...
**Decided:** Nisba. The product was called Kiln for one day.
