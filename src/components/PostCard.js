...
export const PostCard = ({ post, joined, onJoin, onOpenProfile }) => {
    ...
    <Glass style={{ marginBottom: 20, overflow: 'hidden' }}>
      ...
      <View style={{ padding: 13 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
          <Text style={{ color: C.green, fontSize: 12.5, fontWeight: '800' }}>
            ⚡ {post.vibes + (joined ? 1 : 0)} joining
          </Text>
          <Text style={{ color: C.dim, fontSize: 12.5, marginLeft: 14 }}>💬 {post.comments}</Text>
          <View style={{ flex: 1 }} />
