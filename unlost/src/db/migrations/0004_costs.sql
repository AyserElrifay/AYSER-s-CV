...
grant select, insert, update, delete on costs to unlost_role_owner;
grant select, insert on costs to unlost_role_manager;
 ...
 */
grant insert on costs to unlost_role_member;
...
create policy costs_manager_read on costs
  ...
  using (
    org_id = unlost.current_org_id()
    and exists (select 1 from deals d where d.id = costs.deal_id)
  );

create policy costs_manager_write on costs
  for insert to unlost_role_manager
  with check (
    org_id = unlost.current_org_id()
    and recorded_by_user_id = unlost.current_user_id()
    and exists (select 1 from deals d where d.id = costs.deal_id)
  );
...
create policy costs_member_write on costs
  for insert to unlost_role_member
  with check (
    org_id = unlost.current_org_id()
    and recorded_by_user_id = unlost.current_user_id()
  );
...
grant select (cost_drift_alert_bp) on organizations to unlost_role_manager;
