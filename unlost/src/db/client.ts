 ...
 *  - A query that forgets to switch roles is refused outright, because unlost_app
...
function connectionUrl(): string {
  ...
  if (!url) throw new Error('DATABASE_URL is not set. Copy unlost/.env.example to unlost/.env.');
  return url;
}
...
export async function findLoginCandidates(email: string): Promise<LoginCandidate[]> {
  const rows = await getDb().execute<{
    ...
    user_name: string;
    locale: string;
  }>(raw`select * from unlost.authenticate_lookup(${email})`);
  ...
  if (faults.length > 0) {
    throw new TenantContextError(
      `DATABASE_URL connects as "${row.role_name}", which ${faults.join(' and ')}. ` +
        'Point it at unlost_app instead — see unlost/.env.example.',
    );
  }
}
