 ...
 * connects as unlost_app, which owns nothing and inherits nothing.
 */
...
export interface MigrateOptions {
  adminUrl: string;
  /** Password given to unlost_app; the app's own DATABASE_URL must match. */
  appPassword: string;
...
export async function migrate({ adminUrl, appPassword, log = () => {} }: MigrateOptions) {
  ...
  try {
    ...
    await sql`create schema if not exists unlost`;
    await sql`
      create table if not exists unlost.schema_migrations (
    ...
    const applied = new Set(
      (await sql<{ version: string }[]>`select version from unlost.schema_migrations`).map(
        (row) => row.version,
      ),
    );
    ...
    for (const file of files) {
      ...
      await sql.begin(async (tx) => {
        ...
        await tx`insert into unlost.schema_migrations (version) values (${file})`;
      });
      log(`  + ${file}`);
    }
...
async function grantSchemaUsage(sql: postgres.Sql) {
  ...
  for (const role of roles) {
    await sql.unsafe(await statement(sql, 'grant usage on schema unlost to %I', role));
  }
}
...
async function main() {
  ...
  if (!adminUrl || !appPassword) {
    console.error('Set ADMIN_DATABASE_URL and UNLOST_APP_DB_PASSWORD. See unlost/.env.example.');
    process.exit(1);
  }
