...
`../` in this repository is a different project (`moments-superapp`). Unlost lives
...
```bash
sudo -u postgres psql -c "create database unlost_dev;"
sudo -u postgres psql -c "create database unlost_test;"
...
migration creates `unlost_app` and the four role-specific roles itself.
...
3. Run the migration once from your machine:
   ...
   ```bash
   ADMIN_DATABASE_URL='postgres://…' UNLOST_APP_DB_PASSWORD='<a long random string>' \
     npm run db:migrate
   ```

   This creates `unlost_app` and the four role-specific roles. `DATABASE_URL` is
   then the same host and database as the admin URL, but as `unlost_app` with the
   password you just set.

### 2. Application — Vercel

1. Import this repository at [vercel.com](https://vercel.com).
2. **Set Root Directory to `unlost`.** The repository root is a different project.
3. Environment variables:

   | Name | Value |
   |---|---|
   | `DATABASE_URL` | the `unlost_app` connection string, **pooled** |
   | `AUTH_SECRET` | `openssl rand -base64 32` |
   | `AUTH_TRUST_HOST` | `true` |

   `ADMIN_DATABASE_URL` and `UNLOST_APP_DB_PASSWORD` are **not** set on Vercel.
...
src/
  money/          The money module. Pure, no I/O, exhaustively tested.
                  Every piece of money arithmetic in Unlost is in here.
  db/
