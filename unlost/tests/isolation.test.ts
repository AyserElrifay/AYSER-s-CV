 ...
 * unlost_app, the same role the application connects as. Nothing here has more
...
describe('context does not survive its transaction', () => {
  ...
  it('restores the connection role afterwards', async () => {
    await withTenant(as(orgA, 'member'), async (tx) => {
      ...
      expect(Array.from(inside as Iterable<{ role: string }>)[0]?.role).toBe('unlost_role_member');
    });
    // Back on unlost_app, which cannot see the schema at all.
    await expectRefused(
